defmodule TakeaplegeApi.Web.PostController do
  ...
  def create(conn, %{"post" => post_params}) do
    with {:ok, %Post{} = post} <- Category.create_post(post_params) do
      conn
      |> put_status(:created)
  ...
  def update(conn, %{"id" => id, "post" => post_params}) do
    ...
    with {:ok, %Post{} = post} <- Category.update_post(post, post_params) do
      render(conn, "show.json", post: post)
    end
  end
  ...
  def delete(conn, %{"id" => id}) do
    post = Category.get_post!(id)
    with {:ok, %Post{}} <- Category.delete_post(post) do
